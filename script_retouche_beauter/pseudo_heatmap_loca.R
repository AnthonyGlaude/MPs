# CLEANED FULL SCRIPT WITH ENGLISH COMMENTS
...
# Visualization and table export
library(ggplot2)
library(patchwork)
library(flextable)
library(pagedown)

# Input/output and image processing
library(officer)
library(png)
library(grid)

# === PART 1: Load and Prepare Expression Data ===
...
# Merge annotation with pseudogenes
...
# Enrich expression data with annotations
results_list_enriched <- results_list %>%
  left_join(mane_select %>% select(
  ...
  mutate(
    ...
    is_canonical = !is.na(MANE_Transcript) & MANE_Transcript != ""
  ) %>%
...
# === PART 2: Calculate Expression Ratios ===

# Compute ratios per transcript relative to canonical
...
# Summarize by transcript and cluster
...
# 1) Marquer les pseudogènes (à adapter selon vos colonnes)
results_list_enriched <- results_list_enriched %>%
  mutate(is_pseudogene = if_else(grepl("P\\d+$", Gene), TRUE, FALSE))

# 2) Identifier les UniProt canoniques
canonical_uniprots <- results_list_enriched %>%
  filter(is_canonical, !is.na(UniProt_ID)) %>%
  pull(UniProt_ID) %>%
  unique()

# 3) Créer un label de groupe pour chaque transcript/gène
results_list_enriched <- results_list_enriched %>%
...
# 4) Lister les groupes (canonical, protein, non-coding, pseudogenes)
...
for (i in seq_len(nrow(protein_groups))) {
  ...
  # ----------------------------------------------------------------
  # C) Réordonner l'axe y : pour les non pseudogènes, mettre les transcrits canoniques en premier
  # ----------------------------------------------------------------
  if (label != "Pseudogenes") {
  ...
  # ----------------------------------------------------------------
  # D) Création du heatmap principal
  # ----------------------------------------------------------------
  ...
  # ----------------------------------------------------------------
  # E) Création du mini heatmap résumé (moyenne par Tissue pour ce groupe)
  # ----------------------------------------------------------------
  avg_df <- heatmap_df %>%
    group_by(Tissue) %>%
  ...
  # Combinaison du heatmap principal et du mini heatmap
  combined_plot <- heatmap_plot / avg_heatmap +
    plot_layout(heights = c(1, 0.3))
  
  # Stocker le plot dans la liste
  plots_list[[label]] <- combined_plot
}

# --------------------------------------------------------------------
# 6) Réordonner les plots et les combiner en un seul plot vertical
# --------------------------------------------------------------------
